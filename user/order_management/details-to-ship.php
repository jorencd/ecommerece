...
<body>
  ...
  <nav class="navbar bg-body-tertiary fixed-top shadow-sm py-0">
    <div class="container-fluid">
      <!-- Left Arrow Icon -->
      <a class="navbar-brand" href="tracker.php#to-ship">
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (shipDetails) {
        ...
        <div class="card d-flex flex-row p-2">
          <img src="${shipDetails.image}" class="img-fluid" alt="${shipDetails.name}"
            style="width: 100px; height: 100px; object-fit: cover;">
          <div class="card-body text-start p-0 ps-2 mt-2">
            <p class="product-name fw-bold mb-0">${shipDetails.name}</p>
            <p class="mb-1">Color: ${shipDetails.color}</p>
            <p class="mb-3">Quantity: ${shipDetails.quantity}</p>
          </div>
          <div class="card-body p-0 pe-2 text-end price-info">
            <p class="m-0 fw-bold">${shipDetails.status}</p>
            <p class="m-0 mt-5 fw-bold">${shipDetails.price}</p>
          </div>
        </div>`;

        // Parse the price to get the numeric value
        const price = parseFloat(shipDetails.price.replace('Php.', '').replace(',', ''));
        const quantity = shipDetails.quantity;
