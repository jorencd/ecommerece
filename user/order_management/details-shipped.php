...
<script>
  ...
  const shippedDetails = JSON.parse(localStorage.getItem('shippedDetails')); // Corrected key

  if (shippedDetails) {
    const shippedDetailsContainer = document.getElementById('product-details');

    shippedDetailsContainer.innerHTML = `
      <div class="card d-flex flex-row p-2">
        <img src="${shippedDetails.image}" class="img-fluid" alt="${shippedDetails.name}"
          style="width: 100px; height: 100px; object-fit: cover;">
        <div class="card-body text-start p-0 ps-2 mt-2">
          <p class="product-name fw-bold mb-0">${shippedDetails.name}</p> <!-- Corrected variable -->
          <p class="mb-1">Color: ${shippedDetails.color}</p>
          <p class="mb-3">Quantity: ${shippedDetails.quantity}</p>
        </div>
        <div class="card-body p-0 pe-2 text-end price-info">
          <p class="m-0 fw-bold">${shippedDetails.status}</p>
          <p class="m-0 mt-5 fw-bold">${shippedDetails.price}</p>
        </div>
      </div>`;

    // Parse the price to get the numeric value
    const price = parseFloat(shippedDetails.price.replace('Php.', '').replace(',', ''));
    const quantity = shippedDetails.quantity;
