...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    const payDetails = JSON.parse(localStorage.getItem('payDetails'));

    if (payDetails) {
      const payDetailsContainer = document.getElementById('product-details');

      payDetailsContainer.innerHTML = `
          <div class="card d-flex flex-row p-2">
            <img src="${payDetails.image}" class="img-fluid" alt="${payDetails.name}" 
              style="width: 100px; height: 100px; object-fit: cover;">
            <div class="card-body text-start p-0 ps-2 mt-2">
              <p class="product-name fw-bold mb-0">${payDetails.name}</p>
              <p class="mb-1">Color: ${payDetails.color}</p>
              <p class="mb-3">Quantity: ${payDetails.quantity}</p>
            </div>
            <div class="card-body p-0 pe-2 text-end price-info">
              <p class="m-0 fw-bold">${payDetails.status}</p>
              <p class="m-0 mt-5 fw-bold">${payDetails.price}</p>
            </div>
          </div>`;

      const price = parseFloat(payDetails.price.replace('Php.', '').replace(',', ''));
      const subtotal = price * payDetails.quantity;
